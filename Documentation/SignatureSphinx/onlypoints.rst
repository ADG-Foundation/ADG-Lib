...
.. csv-table:: 
   ...
   :delim: #
    ...
    "concyclic(A,B,C,D)"#""#":math:`A`, :math:`B`, :math:`C` and :math:`D` belong to the same circle"#":math:`Coplanar A B C D \land \exists O\; \congruent{OA}{OB} \land \congruent{OA}{OC}  \land \congruent{OA}{OD}`"
    "congruent(A,B,C,D)"#":math:`\congruent{AB}{CD}`"#"the segments :math:`AB` and :math:`CD` are congruent (intuitively in the sense that they have same length, but length measure is not assumed to exist)"#
    "congruentAngles(A,B,C,D,E,F)}"#":math:`\congruentAngles{A}{B}{C}{D}{E}{F}`"#"the angles :math:`\angle{ABC}` and :math:`\angle{DEF}` are congruent"#":math:` A \neq B \land C \neq B \land D \neq E \land F \neq E \land $ \n $ \exists A', \exists C', \exists D', \exists F',  \betweenNonStrict{B}{A}{A'} \land \congruent {AA'}{ED} \land $ \n $ \betweenNonStrict {B}{C}{C'} \land \congruent {CC'}{EF} \land  \betweenNonStrict {E}{D}{D'} \land \congruent {DD'}{BA} \land $ \n $ \betweenNonStrict {E}{F}{F'} \land \congruent {FF'}{BC} \land  \congruent {A'C'}{D'F'}`"
    "congruentCircles(O,P,O',P')"#""#"the two circles are congruent."#
    "congruentTriangles(A,B,C,A',B',C')"#""#":math:`ABC` is congruent to :math:`A'B'C'`"#"$ \congruent{AB}{A'B'} \land \congruent{AC}{A'C'} \land \congruent{BC}{B'C'} $"
    "congruentRectangles(A,B,C,D,A',B',C',D')"#""#":math:`ABCD`  and :math:`A'B'C'D'` are congruent rectangles"#"$ \congruent{AB}{A'B'} \land \congruent{BC}{B'C'} \land Rectangle(A,B,C,D) \land Rectangle(A',B',C',D') $"
    "diameter(A,B,O,P)"#""#":math:`AB` is a diameter of the circle of center :math:`O` going through :math:`P`."#
    "equilateral(A,B,C)"#""#":math:`ABC` is an equilateral triangle"#":math:`\congruent{AB}{BC} \land \congruent{BC}{CA}`"
...
.. "isosceles(A,B,C)"#""#":math:`ABC` is an isosceles triangle in :math:`B`, points may be equal."#"$\congruent{AB}{BC}$"
.. "isoscelesStrict(A,B,C)"#""#":math:`ABC` is an isosceles triangle in :math:`B`, points are not collinear."#"$\congruent{AB}{BC} \land \lnot \col{A}{B}{C} $"
...
.. "{\bf midpoint(M,A,B)}"#"$\midpoint{A}{M}{B}$"#"$M$ is the midpoint of segment :math:`AB`"#"$\betweenNonStrict{A}{M}{B} \land \congruent{A M}{B M} $"
